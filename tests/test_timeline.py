...
def test_timeline_is_deterministic() -> None:
    ...
    observations = [
        Observation(id="o1", arrival=start, departure=start, latitude=27.67, longitude=85.32, media_ids=["m1"]),
        Observation(id="o2", arrival=start + timedelta(hours=2), departure=start + timedelta(hours=2), latitude=27.70, longitude=85.35, media_ids=["m2"]),
    ]
    trip = Trip(
        id="t1",
        started_at=start,
