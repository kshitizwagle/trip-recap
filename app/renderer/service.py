...
class RenderService:
    ...
    def create_job(self, trip_id: str) -> str:
        render_id = str(uuid4())
        directory = self.store.render_dir(render_id)
        self._write_status(directory, {"id": render_id, "trip_id": trip_id, "status": "queued"})
        return render_id

    async def run(self, render_id: str, trip_id: str, preview_url: str) -> None:
        directory = self.store.render_dir(render_id)
        frames = directory / "frames"
        output = directory / "trip.mp4"
        self._write_status(directory, {"id": render_id, "trip_id": trip_id, "status": "rendering"})
        try:
            renderer = BrowserRenderer()
            frame_count, duration = await renderer.capture_frames(preview_url, trip_id, frames)
            await encode_frames(frames, output, renderer.fps)
            self._write_status(
                directory,
                {
                    "id": render_id,
            ...
            self._write_status(
                directory,
                {"id": render_id, "trip_id": trip_id, "status": "failed", "error": str(exc)},
            )
        finally:
    ...
    def video_path(self, render_id: str) -> Path:
        return self.store.render_dir(render_id) / "trip.mp4"
